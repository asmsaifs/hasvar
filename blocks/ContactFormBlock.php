...
/**
 * Block created with Luya Block Creator Version 1.0.0-RC1 at 13.10.2016 09:15
 */
class ContactFormBlock extends \luya\cms\base\PhpBlock
{
    /**
    ...
    {
        return 'email'; // choose icon from: https://design.google.com/icons/
    }
    ...
    public function config()
    {
        return [
            'vars' => [
				...
				['var' => 'subjectPlaceholder', 'label' => 'Place holder for Subject in the email', 'type' => 'zaa-text', 'placeholder' => $this->defaultSubjectPlaceholder],
                ...
                ['var' => 'nameError', 'label' => 'Error for field "Name"', 'type' => 'zaa-text', 'placeholder' => $this->defaultNameError],
                ['var' => 'emailError', 'label' => 'Error for field "Email"', 'type' => 'zaa-text', 'placeholder' => $this->defaultEmailError],
                ['var' => 'messageError', 'label' => 'Error for field "Message"', 'type' => 'zaa-text', 'placeholder' => $this->defaultMessageError],
    ...
    public function extraVars()
    {
        return [
            ...
            'namePlaceholder' => $this->getVarValue('namePlaceholder', $this->defaultNamePlaceholder),
            'nameError' => $this->getVarValue('nameError', $this->defaultNameError),
            'emailLabel' => $this->getVarValue('emailLabel', $this->defaultEmailLabel),
            'emailPlaceholder' => $this->getVarValue('emailPlaceholder', $this->defaultEmailPlaceholder),
            'emailError' => $this->getVarValue('emailError', $this->defaultEmailError),
            'messageLabel' => $this->getVarValue('messageLabel', $this->defaultMessageLabel),
            'messageError' => $this->getVarValue('messageError', $this->defaultMessageError),
            'sendLabel' => $this->getVarValue('sendLabel', $this->defaultSendLabel),
            'sendError' => $this->getVarValue('sendError', $this->defaultSendError),
            'sendSuccess' => $this->getVarValue('sendSuccess', $this->defaultSendSuccess),            
            'subjectLabel' => $this->getVarValue('subjectLabel', $this->defaultMailSubject),
            'subjectPlaceholder' => $this->getVarValue('subjectPlaceholder', $this->defaultSubjectPlaceholder),
            'subjectError' => $this->getVarValue('subjectError', $this->defaultSubjectError),
        ...
        return  '<p><i>Form Block</i></p>{% if vars.emailAddress is not empty %}'.
                    '{% if vars.headline is not empty %}<h3>{{ vars.headline }}</h3>{% endif %}'.
                        '<div class="input input--text">'.
                            '<label for="name" class="input__label">{{ vars.nameLabel }}</label>'.
                            '<div class="input__field-wrapper"><input id="name" class="input__field" disabled="disabled" /></div>'.
                        '</div>'.
                        '<div class="input input--text">'.
                        '<label for="name" class="input__label">{{ vars.emailLabel }}</label>'.
                        '<div class="input__field-wrapper"><input id="name" class="input__field" disabled="disabled" /></div>'.
                        '</div>'.
                        '<div class="input input--text">'.
                        '<label for="name" class="input__label">{{ vars.subjectLabel }}</label>'.
                        '<div class="input__field-wrapper"><input id="name" class="input__field" disabled="disabled" /></div>'.
                        '</div>'.
                        '<div class="input input--text">'.
                        '<label for="name" class="input__label">{{ vars.messageLabel }}</label>'.
                        '<div class="input__field-wrapper"><textarea class="input__field" disabled="disabled" /></div>'.
                        '</div>'.
                        '<button class="btn" disabled>{{ vars.sendLabel }}</button>'.
